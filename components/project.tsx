...
export default function Project({title, description, tags,imageUrl,href} : ProjectProps) {
    ...
    sm:h-[20rem] mb-3 sm:mb-8 last:mb-0 even:pl-8 dark:bg-white/10 dark:hover:bg-white/20 dark:text-white">
        ...
        <div className="pt-4 pb-7 px-5 sm:pl-10 sm:pr-2 sm:pt-10 sm:max-w-[50%] flex flex-col h-full group-even:ml-[18rem] " >
            ...
            <ul className="flex flex-wrap mt-4 gap-2 sm:mt-auto">
                {tags.map((tag, key) => (
                    <li className="bg-gray-950/[0.7] px-3 py-1 text-[1rem] uppercase tracking-wider text-white rounded-full dark:text-white/70" key={key}>{tag}</li>
                ))}
            </ul>
        </div>
            <Link href={href}>
